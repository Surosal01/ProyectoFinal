...
void main()
{
	...
	//vec3 specular = light.specular * spec * vec3(texture(material.specular, TextCoords).rgb);
    
    //vec3 result = ambient + diffuse + specular;

	color = vec4(ambient + diffuse + specular,texture( material.diffuse, TexCoords).a);
	if(color.a < 0.1)
		discard;
